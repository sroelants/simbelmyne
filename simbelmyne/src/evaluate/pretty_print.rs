...
use super::{tuner::NullTrace, Eval, EvalContext};
...
pub fn print_eval(board: &Board) -> String {
    ...
    for (rank, squares) in Square::RANKS.into_iter().enumerate() {
        ...
        for (file, sq) in squares.into_iter().enumerate() {
            ...
            let score = if let Some(piece) = board.get_at(sq) {
                // Get score for piece
                let score = eval.material(piece, &mut NullTrace) + eval.psqt(piece, sq, &mut NullTrace);
    ...
    let white_rook_open_file =  eval.rook_open_file::<WHITE>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    let black_rook_open_file = -eval.rook_open_file::<BLACK>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    lines.push(format!("{:<25} {:>7.2} {:>7.2}", "Rook on open file:", white_rook_open_file, black_rook_open_file));

    let white_rook_semiopen_file =  eval.rook_semiopen_file::<WHITE>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    let black_rook_semiopen_file = -eval.rook_semiopen_file::<BLACK>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    lines.push(format!("{:<25} {:>7.2} {:>7.2}", "Rook on semiopen file:", white_rook_semiopen_file, black_rook_semiopen_file));

    let white_connected_rooks =  eval.connected_rooks::<WHITE>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    let black_connected_rooks = -eval.connected_rooks::<BLACK>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    lines.push(format!("{:<25} {:>7.2} {:>7.2}", "Connected rooks:", white_connected_rooks, black_connected_rooks));

    let white_queen_open_file =  eval.queen_open_file::<WHITE>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    let black_queen_open_file = -eval.queen_open_file::<BLACK>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    lines.push(format!("{:<25} {:>7.2} {:>7.2}", "Queen on open file:", white_queen_open_file, black_queen_open_file));

    let white_queen_semiopen_file =  eval.queen_semiopen_file::<WHITE>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    let black_queen_semiopen_file = -eval.queen_semiopen_file::<BLACK>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    ...
    let white_virtual_mobility =  eval.virtual_mobility::<WHITE>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    let black_virtual_mobility = -eval.virtual_mobility::<BLACK>(&board, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    lines.push(format!("{:<25} {:>7.2} {:>7.2}", "Virtual mobility:", white_virtual_mobility, black_virtual_mobility));

    let white_king_zone =  eval.king_zone::<WHITE>(&mut ctx, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
    let black_king_zone = -eval.king_zone::<BLACK>(&mut ctx, &mut NullTrace).lerp(eval.game_phase) as f32 / 100.0;
