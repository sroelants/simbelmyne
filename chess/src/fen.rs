...
//!
//! Not doing the best job at having clear errors when passed invalid FEN strings,
//! it'll just scream "Invalid!" and blow up. 💥
...
impl Board {
    ...
    pub fn from_fen(fen: &str) -> anyhow::Result<Board> {
        ...
        let half_moves = parts.next()
            .ok_or(anyhow!("Invalid FEN string"))?
            .parse()?;

        let full_moves = parts.next()
            .ok_or(anyhow!("Invalid FEN string"))?
            .parse()?;
