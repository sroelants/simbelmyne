...
mod tests {
    ...
    #[test]
    fn pseudolegal_matches() {
        for fen in TEST_POSITIONS {
            let board: Board = fen.parse().unwrap();
            let legals = board.legal_moves::<All>();
            let pseudos = board.pseudolegal_moves::<All>();

            assert!(pseudos.len() >= legals.len(), "Found less pseudos than legals");

            let valid_pseudos: MoveList = pseudos
                .into_iter()
                .filter(|&mv| board.try_play(mv).is_some())
                .collect();

            assert!(valid_pseudos.len() == legals.len(), "Different number of legals and valid pseudos");

            for valid in valid_pseudos {
                assert!(legals.contains(&valid));
            }
        }
    }
}
