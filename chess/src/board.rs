...
pub struct Board {
  ...
  /// List of pieces, indexable by a Square, more efficient for lookups than
  /// `pieces`
...
impl Board {
  ...
  /// to. These are basically all the squares that are under attack if the
  /// king were taken off the board. (Since the king might be obstructing
  /// some attacks, but it would still be unsafe for the king to move
  /// there).
