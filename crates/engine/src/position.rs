...
mod tests {
  ...
  fn test_hash_updates() {
    ...
    // Check that incremental updates yield the same result as hashing the
    // entire board
  ...
  fn test_faulty_position() {
    ...
    let board = "8/4k3/1p5p/4PB2/1Pr3P1/1K1N4/8/8 b - - 4 53"
      .parse()
      .unwrap();
    let mut position = Position::new(board);
  ...
  fn test_incremental_pawn_hash() {
    ...
    let terminal = Position::new(
      "rnbqkb1r/ppp1pppp/3p1n2/8/4P3/3P4/PPP2PPP/RNBQKBNR w KQkq - 1 3"
        .parse()
        .unwrap(),
    );
