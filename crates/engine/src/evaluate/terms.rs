...
use crate::evaluate::kp_structure::KingPawnStructure;
...
pub fn psqt(
  ...
  let sq = if color.is_white() { sq.flip() } else { sq };

  trace.add(|t| {
    use PieceType::*;
    let term = if color.is_white() { 1 } else { -1 };
    match ptype {
...
pub fn knight_outposts<const US: Color>(
  ...
  let outpost_knights = board.knights(US) & kp_structure.outposts(US);
...
pub fn bishop_outposts<const US: Color>(
  ...
  let perspective = if US.is_white() { 1 } else { -1 };
  let outpost_bishops = board.bishops(US) & kp_structure.outposts(US);
...
pub fn rook_open_file<const US: Color>(
  ...
  let perspective = if US.is_white() { 1 } else { -1 };
  let rooks_on_open = board.rooks(US) & kp_structure.open_files();
...
pub fn rook_semiopen_file<const US: Color>(
  ...
  let perspective = if US.is_white() { 1 } else { -1 };
  let rooks_on_semi = board.rooks(US) & kp_structure.semi_open_files(US);
...
pub fn queen_open_file<const US: Color>(
  ...
  let perspective = if US.is_white() { 1 } else { -1 };
  let queens_on_open = board.queens(US) & kp_structure.open_files();
...
pub fn queen_semiopen_file<const US: Color>(
  ...
  let queens_on_semi = board.queens(US)
    & kp_structure.semi_open_files(US)
    & !kp_structure.open_files();
...
pub fn threats<const US: Color>(
  ...
  for victim in [Pawn, Knight, Bishop, Rook, Queen] {
    total += PARAMS.pawn_attacks[victim]
      * (board.get_bb(victim, !US) & ctx.attacked_by[US][Pawn]).count() as i32;
    ...
    total += PARAMS.rook_attacks[victim]
      * (board.get_bb(victim, !US) & ctx.attacked_by[US][Rook]).count() as i32;
    total += PARAMS.queen_attacks[victim]
      * (board.get_bb(victim, !US) & ctx.attacked_by[US][Queen]).count() as i32;
  }
...
pub fn checks<const US: Color>(
  ...
  let knight_checks = ctx.attacked_by[US][Knight] & knight_squares(their_king);
...
pub fn volatile_passers<const US: Color>(
  ...
  let passers = kp_structure.passed_pawns(US);
