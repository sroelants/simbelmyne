...
pub fn tunable(_attr: TokenStream, item: TokenStream) -> TokenStream {
  ...
  let uci_atomic_idents =
    uci_decls.iter().map(|item| &item.ident).collect::<Vec<_>>();
...
fn parse_uci_attr(attr: &Attribute) -> (i32, i32, i32) {
  ...
  for meta in nested {
    let Meta::NameValue(meta) = meta else {
      panic!("Invalid param passed to uci attr")
    };
...
fn replace_decl(item: &ItemConst) -> impl ToTokens {
  ...
  let getter_ident =
    syn::Ident::new(&ident.to_string().to_ascii_lowercase(), ident.span());
