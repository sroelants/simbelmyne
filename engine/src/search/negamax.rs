...
impl<'a> SearchRunner<'a> {
  ...
  pub fn negamax<const PV: bool>(
    ...
    // 1. More aggressively prune fail-high based pruning/reductions (rfp, nmp,
    //    etc...)
    // 2. Be more cautious with fail-low based pruning/reductions (fp,
    //    alpha-based reductions, etc...)
    //
    ////////////////////////////////////////////////////////////////////////

    let improving =
      !in_check && ply >= 2 && self.stack[ply - 2].eval < static_eval;
    ...
    while let Some(mv) = legal_moves.next(&self.history) {
      ...
      // Instruct the CPU to load the TT entry into the cache ahead of
      // time
      ...
      // increases alpha
          ...
          && move_count >= lmr_threshold() + PV as usize
        {
          ...
          // is high.
          // NOTE: 140 seems to be the max I can ever hit here
          reduction -= (complexity > 130) as i16;
    ...
    // - Decrement all preceding quiets that failed to beat beta in both history
    //   tables
    //
    ////////////////////////////////////////////////////////////////////////
    ...
    if excluded.is_none() {
      ...
      // 2. We have no best move (fail-low), or the best move is a quiet move
      // 3. The score is valid to use when it comes to the node bounds: 3.a) If
      //    the score is >= eval and the score is _not_ an upper bound 3.b) If
      //    the score is <= eval and the score is _not_ a lower bound
      //
      ///////////////////////////////////////////////////////////////////
