...
///
/// These are quiet moves (i.e., not captures or promotions) that still
/// managed to cause a beta cutoff. Think your forks, mates, etc...
/// Killer moves are stored per ply, and and are less "relevant" than more
/// exact information like the TT table (which is as exact as it gets: it's
/// a good move _for this specific position_. Still, a mate in a variation
...
pub struct Killers {
  ...
  // The length up to which we've stored _actual_ moves. Anything beyond
  // `len` is considered garbage.
  len: usize,
}
...
impl Killers {
  ...
  /// Return the length of the killers table (i.e., the number of stored
  /// moves)
